...
public class SpeechInjector {
    ...
    var speechButton : MDCFloatingButton?
    var buttonColor: UIColor!
    var buttonRecordingColor: UIColor!
    ...
    private func _startRecording() throws {
        if !audioEngine.isRunning {
            let timer = Timer(timeInterval: 10.0, target: self, selector: #selector(_timerEnded), userInfo: nil, repeats: false)
            ...
            var triggeredByResult = true // makes sure all custom user actions are peformed only ones!
            recognitionTask = speechRecognizer.recognitionTask(with: recognitionRequest) { result, error in
                ...
                if let result = result {
                    ...
                    NotificationCenter.default.post(name: Notification.Name("didReceiveSpeech"), object: ["Speech": result.bestTranscription.formattedString.lowercased()], userInfo: ["Speech": result.bestTranscription.formattedString.lowercased()])
                    ...
                    self.connectors.forEach({ (sc) in
                        ...
                        for word in sc.words {
                            let currentResult = word.contains(where: receivedSpeech.contains)
                            if currentResult == true {
                                combinedResult = currentResult
                                 break
                            }
                        }
                        print("TriggeredByResult combinedResult \(combinedResult)")

                        if combinedResult,  triggeredByResult {
                            print("triggeredByResult Success")
    ...
    private func _setRecordingColor() {
        NotificationCenter.default.post(name: Notification.Name("setRecordingColor"), object: nil, userInfo: nil)

        speechButton?.backgroundColor = buttonRecordingColor
    }
    
    private func _setButtonColor() {
        NotificationCenter.default.post(name: Notification.Name("setButtonColor"), object: nil, userInfo: nil)
    ...
    public func placeSpeechButton(position : SpeechButtonLocation = .rightBottom,
        ...
        case .leftBottom:
            xPos = xOffset
            yPos = vc!.view.frame.height - (speechButton?.frame.height ?? 0) - yOffset -  _returnSafeAreaSize().bottom
        case .letftTop :
            xPos = xOffset
            yPos = yOffset - _returnSafeAreaSize().top
            
        case .rightBottom :
            xPos = vc!.view.frame.width - (speechButton?.frame.width ?? 0) - xOffset
            yPos = vc!.view.frame.height - (speechButton?.frame.height ?? 0) - yOffset - _returnSafeAreaSize().bottom
            
        case .rightTop :
            xPos = vc!.view.frame.width - (speechButton?.frame.width ?? 0) - xOffset - _returnSafeAreaSize().top
            yPos = yOffset
        }
        
//        vc!.view.addSubview(speechButton? ?? 0)
